...
- Depending on credential type, original credential information (with cryptographic key material) need not to be sent. Instead, sender will be transferring its approval token for Receiver to acquire new credential information (e.g. receiver device may have just a subset of the sender's device access rights or receiver's device credential may be revoked independently of the sender's device credential).
