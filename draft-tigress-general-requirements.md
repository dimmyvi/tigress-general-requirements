...
- Depending on credential type, original credential information (with cryptographic key material) need not be sent. Instead, sender shall be transferring its approval token for Receiver to acquire new credential information (e.g. receiver device may have just a subset of the sender's device access rights or receiver's device credential may be revoked independently of the sender's device credential).
-  Depending on credential type, Provisioning Partner will not allow for two users to use the same credential / cryptographic keys. Credentials on sender and receiver devices are different and are managed independently.
