...
sequenceDiagram
    ...
    actor R as Receiver
    S ->> I : upload Sharing Invititation (CCC)
    break Generic messaging channel
      S ->> R : send invite
    end
    break CCC Credential Provisioning flow
      R ->> I : upload Key Signing Request (CCC)
      S ->> I : read Key Signing Request, generate and upload Key Import Request (CCC)
      R ->> I : read Key Import Request and Import Key Data (CCC)
    end
```
...
- (Req-CredentialType) The solution shall support transfer of various digital credential types, based on symmetric and asymmetric cryptography, public and proprietary standards - including, but not limited, digital car key, MiFare Desfire, SeOS, ISO 18013-5, Unified Access credentials.
